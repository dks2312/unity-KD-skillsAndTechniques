...
public class AT_Turret : MonoBehaviour
{
    ...
    [SerializeField] Transform m_tfchd = null;
    ...
    { 
        ...
        if(t_cols.Length > 0){
            ...
            foreach(Collider t_clTarget in t_cols){
                float t_distance = Vector3.SqrMagnitude(transform.position - t_clTarget.transform.position);
                if(t_shortestDistance > t_distance){
                    t_shortestDistance = t_distance;
                    t_shortestTarget = t_clTarget.transform;
                }
            }
        }
        ...
        if(m_tfTarget == null){
            ...
            if(Quaternion.Angle(m_tfGunBody.rotation, t_fireRotation) < 5f){
                ...
                if(m_currentFireRate <= 0){
                    m_currentFireRate = m_fireRate;
                    Instantiate(m_goPrefab, m_tfchd.position, m_tfchd.rotation);
                }
            }
        }
    }
